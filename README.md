...
Featuring native support for **OpenAI** and **Google Gemini** multimodal vision models, as well as **Azure AI Document Intelligence**, MarkItDown Studio can transcribe complex charts, extract tabular data, and describe visual content automatically.
...
*   **🖼️ Images to Markdown (Dedicated AI Vision)**:
    ...
    *   **AI Vision Providers**: Fully supports **OpenAI** (`gpt-4o`) and **Google Gemini** (`gemini-3.6-flash`, `gemini-flash-latest` with a free tier from Google AI Studio).
    *   **Live Status Indicator**: Visual badge displays whether AI Vision is currently active or needs an API key configured.
...
*   **🌐 Webpage & YouTube URLs**:
    ...
    *   **Service Indicators**: Displays real-time status of Azure AI and OpenAI / Google Gemini Vision integrations.
...
*   **Internet Connection**: Required for converting Webpage / YouTube URLs, and for OpenAI / Google Gemini Vision or Azure AI integrations. Standard local document conversions (PDF, Word, Excel, EPubs, etc.) work completely offline.
...
### 1. Multimodal LLM Vision (OpenAI & Google Gemini)
Toggle on **"Enable LLM Descriptions"** to generate rich visual captions, layout transcriptions, and diagram-to-markdown extractions.

#### Using OpenAI:
*   **API Key**: Enter your OpenAI API key (`sk-...`).
*   **Base URL**: Leave blank (uses default OpenAI endpoints).
*   **Model**: `gpt-4o` or `gpt-4o-mini`.

#### Using Google Gemini (Free Tier Available):
You can use a free Google Gemini key via Google's official OpenAI-compatible endpoint:
*   **API Key**: Paste your Google Gemini API key from [Google AI Studio](https://aistudio.google.com/apikey).
*   **Base URL**: Enter `https://generativelanguage.googleapis.com/v1beta/openai/`
*   **Model**: Enter `gemini-3.6-flash` or `gemini-flash-latest`.

#### Custom OpenAI-Compatible Endpoints:
You can also connect to any OpenAI-compatible provider (e.g., Groq, Ollama, OpenRouter, LocalAI) by entering their API key, Base URL, and model name.

*   **Default Prompt**: Customize the default system prompt used to analyze images across the app (default: *"Write a detailed caption for this image."*).
